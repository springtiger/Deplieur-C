...
int sauveLanguettes(Lang * sL, int nbL, int mode) {
  char * nomFichierDonnees = (dd.fichierDAT[0] == '\0')
...
int sauveDonnees(DonneesDep d) {
  char * nomFichierDonnees = (dd.fichierDAT[0] == '\0')
    ? g_build_filename(g_get_tmp_dir(), "donnees.dep", (gchar *) 0) : dd.fichierDAT;
...
void afficheNum(cairo_t *cr, int num, Vector2d p1, Vector2d p2, Coul c, int typeA) {
...
void afficheNumsPage(cairo_t *cr, AN *lAN, int nbAN, Vector2d v[][3]) {
	for (int iAN = 0; iAN < nbAN; iAN++) {
		...
		if ( ((iAN > 0) && (lANc.n == lAN[iAN-1].n)) || ((iAN < nbAN-1) && (lANc.n == lAN[iAN+1].n)))
			...
			lC = C_NOIR;
		afficheNum(cr, lANc.n, lANc.p1, lANc.p2, lC, 0);
	}
}*/
void faitLigne(cairo_t *cr, Vector2d p1, Vector2d p2, int typeL, int hLang) {
	if (typeL != L_PLI_C) {	// pas de ligne si pli coplanaire
		...
		if ((typeL == L_LGT_C) || (typeL == L_LGT_M) || (typeL == L_LGT_V)) {
			Vector2d pts[4];
			trapeze(pts, p1, p2, hLang*dd.fz, 0.45);
...
DonneesDep chargeOBJ(DonneesDep dd) {
  ...
  g_free(dd.lSNA);
    ...
    for (int j = 0; j < 3; j++) {
      ...
      if (rechN == NULL) {
        dd.lSNA[dd.nAff].nMin = nMin;
        dd.lSNA[dd.nAff].nMax = nMax;
        dd.lSNA[dd.nAff].a = nA++;
        dd.nAff++;
      }
    }
	}
...
*/
  ...
  int dOrig = 9999;
	...
	for (int i = 0; i < dd.nbD; i++) {
      ...
      NAff cleN;
      NAff* rechN;
      cleN.nMax = max(tc, dd.voisins[tc][j].nF);
      cleN.nMin = min(tc, dd.voisins[tc][j].nF);
      rechN = (NAff*)bsearch(&cleN, dd.lSNA, dd.nAff, sizeof(NAff), compAff);
		  int nA = rechN->a;
...
int inversePiece(Depliage* SDi, tB* B, int nbB, int ln0, int ln3) {
  // Init. du dépliage
  SDi[0].orig = dd.sD[ln0].orig;
  SDi[0].face = B[0].face;
  int nbSDi = 1;
...
static void pressed (GtkGestureClick *gesture, int n_press, double x, double y, GtkWidget *area) {
  ...
  if (dd.mode != MODE_PIECE) {
    ...
    if (OK) {
      ...
      int estArete = dd.rects[trouve].type == 1;
      ...
      if ((dd.mode == MODE_ARETE) && estArete) {
        int piece1 = dd.sD[ln1].piece;
        int piece2 = dd.sD[ln2].piece;
        int page1 = dd.sD[ln1].page;
        if (dd.sD[ln1].piece == piece2) { // Deplacement au sein d'une meme piece
          if (ln2 > ln1) {
            //puts("depl SIMPLE");
            ...
            for (int i = 0; i < nbB; i++) {
              B[i].face = dd.sD[ln3 - i].face;
              B[i].orig = (B[i].face == face2) ? face1 : dd.sD[ln3 - i].orig;
              B[i].mis = 0;
            }
            Depliage SDi[nbB];
            inversePiece(SDi, B, nbB, ln0, ln3);

            for (int i = ln0; i <= ln3; i++){
              //if (SDi[i-ln0].face != SDi[i-ln0].orig) {
                dd.sD[i].face = SDi[i-ln0].face;
                dd.sD[i].orig = SDi[i-ln0].orig;
              //}
            }
          }
          ...
          for (int i = 0; i < nbB; i++) {
            ...
            B[in].orig = dd.sD[i+ln0].orig;
            B[in].face = dd.sD[i+ln0].face;
            B[in].mis = 0;
          }
          ...
          if (ln1 < ln2) {
            if (ln1 != ln0) {
              nbSDi = inversePiece(SDi, B, nbB, ln0, ln3);
              SDi[0].orig = face1;
              ...
              for (int i = 0, j = ln4+1; i < nbSDi; i++)
                if (SDi[i].face >= 0) {
                  if (SDi[i].orig != SDi[i].face) {
                    dd.sD[j].orig = SDi[i].orig;
                    dd.sD[j].face = SDi[i].face;
                    dd.sD[j].page = page1;
                    dd.sD[j].piece = piece1;
                    j++;
                  }
                }
            }
            ...
            if (ln2 != ln0) {
              ...
              for (int i = 1; i < nbSDi; i++){
                SDi[i].orig = dd.sD[ln0 +i].orig;
                SDi[i].face = dd.sD[ln0 +i].face;
              }
            }
            /*puts("-sDi-");
            for (int i = 0; i < nbSDi; i++) {
            ...
            for (int i = 0; i < nbSDi; i++)
              if (SDi[i].face >= 0) {
                if (SDi[i].orig != SDi[i].face) {
                  dd.sD[nb].orig = SDi[i].orig;
                  dd.sD[nb].face = SDi[i].face;
                  dd.sD[nb].page = page1;
                  dd.sD[nb].piece = piece1;
                  nb++;
                }
              }
          }
      ...
      } else if ((dd.mode == MODE_ARETE) && !estArete) { // scission
        //printf("SCISSION: %d (%d) %d (%d)\n", ln1, face1, ln2, face2);
        int piece = dd.sD[ln1].piece;
        int numPieces[dd.nbD];
        numPieces[0] = 0;
        for (int i = 0; i < dd.nbD; i++) numPieces[i] = 0;
        for (int i = 0; i < dd.nbD; i++) numPieces[dd.sD[i].piece] = 1;
        int nPLibre;
        ...
        // FIN Scission
      } else if ((dd.mode == MODE_LANG) && estArete) {
        int L1 = -1, L2 = -1;
...
static void exporteDepliage (GtkDialog *dialog, int response) {
...
static void exportePDF (GtkDialog *dialog, int response) {
  if (response == GTK_RESPONSE_ACCEPT) {
    GtkFileChooser *chooser = GTK_FILE_CHOOSER (dialog);
    g_autoptr(GFile) file = gtk_file_chooser_get_file (chooser);
    sauvePDF(file);
...
static void sauveDepliage(GFile * file) {
  strcpy(dd.fichierDAT, g_file_get_path(file));
  sauveDonnees(dd);
  sauveLanguettes(dd.sLgt, dd.nbLgt, M_LANG_SAUV);

  guint id = gtk_statusbar_get_context_id(GTK_STATUSBAR(dd.statut), "sauveDonnees");
  gtk_statusbar_push(GTK_STATUSBAR(dd.statut), id, dd.fichierDAT);
}

static void sauvePDF(GFile * file) {
	cairo_surface_t * surface;
	cairo_t * cr;

  Vector2d pageDim = formats[dd.formatPage];

  guint id = gtk_statusbar_get_context_id(GTK_STATUSBAR(dd.statut), "exportePDF");
  gtk_statusbar_push(GTK_STATUSBAR(dd.statut), id, g_file_get_path(file));
  ...
  for (int i = 0; i < dd.nbL; i++)
    nbPieces = max(nbPieces, dd.lignes[i].nPP);
  ...
  for (int i = 0; i < dd.nbL; i++) {
    ...
    if (l.id > -1) {
      ...
      if (l.nb == 1) {
        memePiece = FALSE;
        for (int li = 0; li < dd.nbL; li++) {
          Ligne l2 = dd.lignes[li];
        ...
        afficheNum(cr, nAA, l.p1, l.p2, memePiece ? C_BLEU : C_NOIR, 0);
        if (dd.nbLgt == 0)
          typeL = L_COUPE;
        else {
          Lang * rL = NULL;
...
static void dessinePage (GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer data) {
  ...
  cairo_rectangle(cr, 10, 10, (formats[dd.formatPage].x -20)* dd.fz, (formats[dd.formatPage].y -20)*dd.fz);
  cairo_stroke(cr);

  cairo_set_font_size(cr, dd.tailleNums*dd.fz);
  ...
  for (int i = 0; i < dd.nbL; i++) {
    ...
    if ((l.id > -1) && (l.nP == nPage)) {
      ...
      _Bool cop0 = fabs(cop) < 10e-7;
      if (l.nb == 1) {
        afficheNum(cr, l.nA, Vector2dMul(l.p1, dd.fz), Vector2dMul(l.p2, dd.fz), C_NOIR, 0);
        if (dd.nbLgt == 0)
          typeL = L_COUPE;
        else {
          Lang * rL = NULL;
      ...
      if (dd.idCourant > -1){
        ...
        Vector2d cZ = dd.coords;
        cairo_move_to(cr, cZ.x, cZ.y);
        cairo_arc(cr, cZ.x, cZ.y, 3 *dd.fz, 0, 2 * pi);
        cairo_stroke(cr);
      }
      cairo_set_line_width(cr, (dd.idCourant == l.nPP ? 2 : 1) *dd.fz);
      Vector2d l1z = Vector2dMul(l.p1, dd.fz);
      Vector2d l2z = Vector2dMul(l.p2, dd.fz);
      faitLigne(cr, l1z, l2z, typeL, dd.hauteurLang);
      ...
      if (dd.mode != MODE_PIECE) {
        ...
        int ln1 = -1, ln2 = -1;
        int p1 = -1, p2 = -1;
        for (int di = 0; di < dd.nbD; di++) {
          int fn = dd.sD[di].face;
          if (fn == l.n1) {
            ln1 = di;
            p1 = dd.sD[ln1].piece;
          } else if (fn == l.n2) {
        ...
        if (type > 0) {
          ...
          dd.nbRects++;
          afficheNum(cr, l.nA, l1z, l2z, C_NOIR, r.type);
        }
      }
    }
  }
...
void ajouteTriangle(int id, Vector2d p1, Vector2d p2, Vector2d p3) {
  p1 = Vector2dMul(p1, dd.fz);
  p2 = Vector2dMul(p2, dd.fz);
  p3 = Vector2dMul(p3, dd.fz);
...
void rendu(int pc) {
  ...
  for (unsigned i = 0; i < nbP; i++) {
    ...
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(frame), dd.dpage[i]);
    gtk_widget_set_vexpand(frame, TRUE);
    gtk_widget_set_hexpand(frame, TRUE);
    gtk_notebook_append_page(d, frame, dd.dlabel[i]);
...
static void ouvreDepliage (GtkDialog *dialog, int response) {
  if (response == GTK_RESPONSE_ACCEPT)
    {
      GtkFileChooser *chooser = GTK_FILE_CHOOSER (dialog);
      g_autoptr(GFile) file = gtk_file_chooser_get_file (chooser);
...
static void open_activated(GSimpleAction *action, GVariant *parameter, gpointer user_data) {
  ...
  g_signal_connect (dialog, "response", G_CALLBACK (ouvreDepliage), NULL);
}
...
static void save_activated(GSimpleAction *action, GVariant *parameter, gpointer user_data) {
  ...
  gtk_widget_show (dialog);
  g_signal_connect (dialog, "response", G_CALLBACK (exporteDepliage), NULL);
}
...
static void expg_activated(GSimpleAction *action, GVariant *parameter, gpointer user_data) {
  ...
  gtk_widget_show (dialog);
  g_signal_connect (dialog, "response", G_CALLBACK (exportePDF), NULL);
}
...
static void zoom_in_clicked (GtkEntry* self, gpointer user_data) {
  dd.fz = dd.fz * 1.25;
  rendu(gtk_notebook_get_current_page(GTK_NOTEBOOK(dd.doc)));
}
...
static void appConfigure (GApplication *app, gpointer user_data) {
  ...
  gtk_window_set_default_size (GTK_WINDOW (win), 670, 1000);
  ...
  gtk_notebook_set_tab_pos(GTK_NOTEBOOK(dd.doc), GTK_POS_LEFT);
  ...
  bModeP = gtk_toggle_button_new_with_label(gtexte(31));
  gtk_box_append (GTK_BOX (box), bModeP);
  bModeF = gtk_toggle_button_new_with_label(gtexte(32));
  gtk_box_append (GTK_BOX (box), bModeF);
  bModeL = gtk_toggle_button_new_with_label(gtexte(33));
  ...
  strcpy(dd.fichierDAT, "");

  guint stid = gtk_statusbar_get_context_id(GTK_STATUSBAR(dd.statut), "...");
  gtk_statusbar_push(GTK_STATUSBAR(dd.statut), stid, textes[35]);
